...
namespace Glab.Utilities
{
    public static class TreeUtils
    {
        ...
        public static void ValidateTreeStructure<TRef, TTarget>(
        ...
        where TTarget : IGH_Goo
        {
                ...
                if (targetTree == null || targetTree.IsEmpty)
                {
                    if (matchTreeStructure)
                    {
                        ...
                        for (int i = 0; i < refTree.Paths.Count; i++)
                        {
                            ...
                            targetTree.EnsurePath(path);
                            // Add the default value in each branch, even if it's null
                            targetTree.Append(defaultValue, path);
                        }
                    }
                    else
                    {
                        // Create a single branch with the default value
                        targetTree.Clear(); // Clear any existing data
                        targetTree.Append(defaultValue, new GH_Path(0));
                    }
                }
            }
            ...
            if (checkEmptyBranches)
            {
                ...
                {
                    var branch = targetTree.get_Branch(path) as List<TTarget>;
                    if (branch.Count == 0)
                    {
            ...
            if (simplifyTree)
            {
                var simplifiedTree = SimplifyTree(targetTree);
                targetTree.Clear(); // Clear the original tree
                foreach (var path in simplifiedTree.Paths)
                {
                    targetTree.AppendRange(simplifiedTree.get_Branch(path).Cast<TTarget>(), path);
                }
            }
            ...
            {
                for (int i = 0; i < targetTree.Paths.Count; i++)
                {
                    var path = targetTree.Paths[i];
                    var branch = targetTree.get_Branch(path) as List<TTarget>;
            ...
            {
                if (refTree.Paths.Count != targetTree.Paths.Count)
                {
                ...
                for (int i = 0; i < refTree.Paths.Count; i++)
                {
                    ...
                    GH_Path pathTar = targetTree.Paths[i];

                    // Get the branches
                    List<TRef> branchRef = refTree.get_Branch(pathRef) as List<TRef>;
                    List<TTarget> branchTar = targetTree.get_Branch(pathTar) as List<TTarget>;
